        ...
        testButton.setOnClickListener {
            ...
            when {
                isYouTubeUrl(url) -> {
                    ...
                    showYouTubeQualityDialog(url) { selectedUrl ->
                        showDebugDialog("Starting YouTube extraction...")
                        extractYouTubeM3U8(selectedUrl) { result ->
                            when {
                                result.success && result.m3u8Url != null -> {
                                    ...
                                    saveOriginalUrl(selectedUrl, result.m3u8Url)
    ...
    private fun showYouTubeQualityDialog(youtubeUrl: String, callback: (String) -> Unit) {
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(qualityNames, savedQualityIndex) { dialog, which ->
                ...
                val selectedFormat = qualities[which].second
                
                // Create format-specific URL (this is conceptual - actual implementation depends on extraction method)
                callback(youtubeUrl) // Pass the original URL with quality preference saved
                dialog.dismiss()
            }
