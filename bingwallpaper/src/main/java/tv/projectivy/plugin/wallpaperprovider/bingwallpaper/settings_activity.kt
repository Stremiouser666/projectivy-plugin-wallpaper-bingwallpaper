    ...
    private suspend fun tryInnerTubeExtraction(videoId: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Try HLS manifest first (best for live streams)
                ...
                // Try progressive formats (video+audio combined)
                ...
                // Try adaptive formats (video-only streams)
                val adaptiveFormats = streamingData.optJSONArray("adaptiveFormats")
                if (adaptiveFormats != null && adaptiveFormats.length() > 0) {
                    // First try to find format with audio
                    ...
                    // Fallback to video-only if no audio found
                    for (i in 0 until adaptiveFormats.length()) {
                        val format = adaptiveFormats.getJSONObject(i)
                        val url = format.optString("url")
                        val mimeType = format.optString("mimeType", "")
    ...
    private suspend fun tryHtmlExtraction(videoId: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Try formats with m3u8/manifest
                val formats = streamingData.optJSONArray("formats")
                if (formats != null) {
                    for (i in 0 until formats.length()) {
                        val format = formats.getJSONObject(i)
